from goods.shelfgoods.utils import send_request
from goods.shelfgoods.utils import http_util
import logging
logger = logging.getLogger("django")

def post_goodgetn(new_image_paths):
    # 调用批量获取图片的相似topn_upc的接口， 获取within_upc
    api, post_data = send_request.get_post_data_goodstopn(new_image_paths)
    reponse_data = None
    if api == None:
...
def post_addgood(upc,image_path,goods_shelfgoods_id):
    # 调用批量获取图片的相似topn_upc的接口， 获取within_upc
    api, post_data = send_request.get_post_data_addgood(upc,image_path,goods_shelfgoods_id)
    reponse_data = None
    if api == None:
...
def post_deletegood(goods_shelfgoods_id):
    # 调用批量获取图片的相似topn_upc的接口， 获取within_upc
    api, post_data = send_request.get_post_data_deletegood(goods_shelfgoods_id)
    reponse_data = None
    if api == None:
