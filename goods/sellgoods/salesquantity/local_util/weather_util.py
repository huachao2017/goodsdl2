...
def get_old_weather(start_date=None):
    ...
    cursor_dmstore.execute('select DISTINCT(owned_city) from shop')
    ...
    if min_create_date is None and max_create_date is None:
        ...
        while True:
            ...
            for city in results:
                if city[0] == '':
                    continue
                weather_ins = get_old_weather_http(city[0],start_date)
        ...
        date2 = datetime.datetime.strptime(start_date, "%Y-%m-%d")
        if date2 < date1:
            i=0
            while True:
                start_date1 = str(
