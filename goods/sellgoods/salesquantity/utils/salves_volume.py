...
class Salves:
    ...
    # 用于upc的特征训练  经过平均编码
    def generate_features5(self,start_time,end_time1,end_time2):
        ...
        feature, df = self.get_test_feature(MeanEncoder,end_time1,end_time2)
    ...
    def get_test_feature(self,MeanEncoder,end_time1,end_time2):
        ...
        sql_dataf = self.sqlsc.sql(
            "select * from (select T4.ai_nums/100 as ai_day_nums,T5.ai_nums/100 as ai_next_nums,T4.ai_shop_id as shop_id,T4.ai_upc as upc ,T4.ai_create_date as ai_day,T5.ai_create_date as ai_nextday,T5.ai_week_date as ai_weekday from salves_volume_day_test T4 left join salves_volume_day_test T5 on T4.ai_shop_id  = T5.ai_shop_id  and T4.ai_upc = T5.ai_upc and T4.ai_next_date = T5.ai_create_date ) T6 where T6.ai_day_nums is not null and T6.ai_next_nums is not null and T6.shop_id is not null and T6.ai_day is not null and T6.ai_nextday is not null and T6.upc != '' and T6.ai_weekday is not null and T6.ai_day_nums < 1 and T6.ai_next_nums < 1 ")
        sql_dataf.show(10)
