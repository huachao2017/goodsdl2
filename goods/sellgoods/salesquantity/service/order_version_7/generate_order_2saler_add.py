        ...
        for mch_code  in result:
            ...
            if drg_ins.delivery_type == 2:
                if drg_ins.upc_status_type == 0:# 首次订货
                    ...
                    drg_ins.safe_stock = max(drg_ins.min_disnums,drg_ins.upc_psd_nums_avg_4, drg_ins.upc_psd_nums_avg_1)
                    ...
                    safe_stock = max(drg_ins.min_disnums, drg_ins.upc_psd_nums_avg_4,
                                             drg_ins.upc_psd_nums_avg_1)
                    track_stock =drg_ins.upc_psd_nums_avg_4 * bhuo_days + safe_stock
                ...
                if drg_ins.upc_status_type ==1 or drg_ins.upc_status_type ==0 :
                    ...
                    end_safe_stock = drg_ins.min_disnums
                    loss_oneday_nums = drg_ins.loss_avg * drg_ins.upc_psd_nums_avg_1 / (1- drg_ins.loss_avg)
                    fudong_nums = 0
                    ...
                    if drg_ins.storage_day < 2:
                        ...
                        safe_day = bhuo_days
                    track_stock = safe_day * drg_ins.upc_psd_nums_avg_1 + fudong_nums + end_safe_stock
                    order_sale = math.ceil(track_stock)
                    drg_ins.safe_stock = end_safe_stock
                    # TODO  商品的psd*保质期＜1，则该商品建议剔除选品，不订货    目前psd 取不到真实值， 这段逻辑先不走
                    if drg_ins.upc_psd_nums_avg_1 * drg_ins.storage_day < 1:
                        order_sale = 0
