            ...
            if drg_ins.delivery_type == 2:
                ...
                if drg_ins.up_status == 1: # 新品
                    ...
                    if drg_ins.delivery_type == 2: #非日配
                        order_sale = order_sale - max(0,drg_ins.stock) - max(0,drg_ins.sub_count) - drg_ins.supply_stock
                else:
                    if drg_ins.psd_nums_4 > 0:
                        x = drg_ins.psd_nums_4 * 2.5 + drg_ins.min_disnums
                    else:
                ...
                if drg_ins.up_status == 1:
                    ...
                    order_sale = math.ceil(track_stock) - max(0, drg_ins.stock) - max(0,
                                                                                      drg_ins.supply_stock) - drg_ins.sub_count
            if order_sale <= 0:
                continue
            # print ("规则2： 起订量规则")
