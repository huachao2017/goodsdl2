...
def many_sort(drg_ins1, drg_ins2):
    psd_nums_4_1 =max(drg_ins1.psd_nums_4,float(drg_ins1.upc_psd_nums_avg_4))
    psd_nums_4_2 = max(drg_ins2.psd_nums_4, float(drg_ins2.upc_psd_nums_avg_4))
...
def bingql_filter(drg_inss,order_data_dict):
    ...
    for drg_ins in drg_inss:
        if drg_ins.category2_id == 124:
            ...
            V1 += drg_ins.height * drg_ins.width * drg_ins.depth * math.ceil(order_sale+drg_ins.sub_count - save_goods_days * float(drg_ins.upc_psd_nums_avg_1))
            for shelf_ins in shelf_inss:
