...
class DailyChangeGoods:
    ...
    def __init__(self,shop_id,template_shop_ids,batch_id,uc_shopid,topn_ratio=0.6,days=28):
        ...
        self.shop_daily_goods_len = 0   # 本店日配商品的总长
        self.shop_not_daily_goods_len = 0    # 本店非日配商品的总长
        self.daily_must_up_ratio = None    # 本店日配商品必上占其总长的比例
        self.not_daily_must_up_ratio = None   # 本店非日配商品必上占其总长的比例
    ...
    def recommend_03(self):
        ...
        # 2.1非日配的下架品、不动品和可选下架品的计算
        not_move_goods, optional_out_goods, daily_not_move_goods,daily_optional_out_goods,must_out_goods = self.calculate_this_shop_old_goods_data(taizhang_goods)
    ...
    def calculate_this_shop_new_goods_data(self,not_move_goods,optional_out_goods):
        ...
        # 2、计算新增品
        # daily_must_up_goods_len = math.ceil(self.all_goods_len * 0.03)
            ...
            try:
                if str(goods[4]) in self.can_order_mch_code_new_dict:
                    if goods[-2] != 301:  # 目前，畅销品和结构品都为必上品
                        if self.can_order_mch_code_new_dict[str(goods[4])][0] == 1:
                            if len(daily_must_up_goods) < self.shop_daily_goods_len * self.daily_must_up_ratio:
                                daily_must_up_goods.append(goods)
                            else:
                                optional_up_goods.append(goods)
                        if self.can_order_mch_code_new_dict[str(goods[4])][0] == 2:
                            if len(not_daily_must_up_goods) < self.shop_not_daily_goods_len * self.not_daily_must_up_ratio:
                                not_daily_must_up_goods.append(goods)
                            else:
                                optional_up_goods.append(goods)
        ...
        # 添加ranking的值
        must_up_goods = self.must_up_add_ranking(must_up_goods)
...
if __name__ == '__main__':
    ...
    f = DailyChangeGoods(1284, "1284,3955,3779,1925,4076,1924,3598,223,4004",'lishu_test_203',806)
